...
# --- Database Lock (defined early so all functions can use it) ---
...
def _logic_statistics(message):
    ...
    for script_key_iter, script_info_iter in list(bot_scripts.items()):
        s_owner_id = script_info_iter.get('script_owner_id', 0)  # Get owner_id from dict, not by splitting key
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'noop': bot.answer_callback_query(call.id)  # Page number button - do nothing
        else:
...
def bot_performance_callback(call):
    ...
    try:
        ...
        performance_parts.append(f"• Uptime Ratio: {running_scripts}/{total_files} ({(running_scripts/total_files*100):.1f}% running)" if total_files > 0 else f"• Uptime Ratio: {running_scripts}/{total_files} (0%)")
